...
static double
_calculateComplex(Triangle *r) {
  return r->w * r->multiplier + r->fn->diagonal(r) * r->w;
}

static double
_calculateAreaIntense(Triangle *r) {
  return r->h * r->multiplier + r->fn->diagonal(r) * 51;
}

static double
_calculateAreaAlt(Triangle *r) {
...
#define Triangle_IMPLEMENTED_METHODS Triangle_PRIVATE_METHODS Triangle_PUBLIC_METHODS
...
register_Triangle(void) {
  iam_Kernel_register(Triangle_prototype);
}
